...
void do_uptime(dbref player)
{
  ...
  send_message(player, "    Muse boot time..: %s",
    mktm(muse_up_time, "D", player));
  send_message(player, "    Current time....: %s",
    mktm(now, "D", player));
  send_message(player, format, "    In operation for:",
    (int) a, (int) b, (int) c, (int) d);
  send_message(player, "");
  send_message(player, "    Next database @dump at: %s",
         mktm(next_dump, "D", player));
  send_message(player, "    Next old mail clear at: %s",
